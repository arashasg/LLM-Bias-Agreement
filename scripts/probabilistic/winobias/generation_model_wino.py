...
import os
import json
import dataloader_wino as dataloader
import math
...
class RedditModel(object):
    ...
    def initialize_model(self):
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            # attn_implementation="flash_attention_2", # Keep commented if Llama 4 fails
            trust_remote_code=True,
            cache_dir="/datasets/llms/",
            device_map="auto" # Fixes OOM
        )
        return model
    ...
    def add_perplexity_scores(self):
        ...
        for i, sample in enumerate(tqdm(self.samples, desc="Processing Wino samples", unit="sample")):
    ...
    def store_samples_as_json(self):
        ...
        for sample in self.samples:
            output_data.append({
                ...
                # Wino Specific Fields
                "occupation": getattr(sample, 'occupation', None),
                "group": getattr(sample, 'group', None),
                "occupation_gender": getattr(sample, 'occupation_gender', None),
                # Scores
