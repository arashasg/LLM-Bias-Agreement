...
class IntersentenceLoader(object):
    def __init__(self, tokenizer, max_seq_length=None, pad_to_max_length=False, input_file="data/bold/prompts"):
...
class StereoSet(object):
    def __init__(self, location):
        ...
        Parameters
        ----------
        location (string): A directory containing JSON files 
                           (e.g., "race_prompt.json").
        """
        
        self.data = []
        
        # Find all files in the directory that match the pattern *_prompt.json
        file_paths = glob.glob(os.path.join(location, '*_prompt.json'))
        
        # Process each file found
        for file_path in file_paths:
            # Extract the 'type' from the filename
            # e.g., "race_prompt.json" -> "race"
            filename = os.path.basename(file_path)
            type_name = filename.split('_prompt.json')[0]
            
            with open(file_path, 'r') as f:
                file_content = json.load(f)
                
                # Iterate through the main keys (e.g., "Asian_Americans")
                for subtype, focuses in file_content.items():
                    
                    # Iterate through the sub-keys (e.g., "Jaegwon_Kim")
                    for focus, texts in focuses.items():
                        
                        # Iterate through each text entry in the array
                        for index, text in enumerate(texts):
                            
                            # Create a unique ID for each sample
                            sample_id = f"{type_name}_{subtype}_{focus}_{index}"
                            
                            # Create the structured sample
                            sample = {
                                "id": sample_id,
                                "type": type_name,
                                "subtype": subtype,
...
#     # 1. A directory (e.g., "./my_data_dir")
#     # 2. A file inside it named "race_prompt.json" with the content you provided.
    
#     # Create a dummy directory and file for testing
#     TEST_DIR = "data/bold/prompts"

#     # --- Load the data using the updated class ---
#     print(f"Loading data from: {TEST_DIR}")
#     dataset = StereoSet(location=TEST_DIR)
    
#     # Print the first 5 samples to verify the structure
#     print("\n--- First 5 Samples ---")
#     num_to_sample = min(5, len(dataset.data))
        
#     if num_to_sample > 0:
#         # Select k random samples from the data
#         random_samples = random.sample(dataset.data, num_to_sample)
        
#         # Print the random samples
#         for sample in random_samples:
#             print(json.dumps(sample, indent=2))
    
#     print(f"\nTotal samples loaded: {len(dataset.data)}")
