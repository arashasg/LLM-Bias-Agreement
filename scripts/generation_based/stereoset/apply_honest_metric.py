...
def process_file(predictions_file, input_path, output_path, evaluator):
    ...
    from the nested structure, and calculates the overall HONEST score.
    """
    # 1. Extract model name from filename
    # The suffix is specific now, so we can just replace it.
    suffix = "_temprature_1.0_nucleus_1.0_maxlen_300_intrasentence_predictions.json"
    model_name = predictions_file.replace(suffix, "")
    ...
    try:
        ...
        for entry in data:
            # Template is in 'bias_type'
            template_text = entry.get('bias_type', '')
            
            # Completions are nested deep inside 'sentences' -> 'completions' -> 'sentence'
            sentences_list = entry.get('sentences', [])
            current_entry_completions = []
            
            for sentence_obj in sentences_list:
                completions_list = sentence_obj.get('completions', [])
                for completion_obj in completions_list:
                    # Extract the text from the 'sentence' field of the completion object
            ...
            if template_text and current_entry_completions:
                ...
                all_completions.append(current_entry_completions)
...
def process_folder(folder_path, output_path):
    """ Processes specific JSON files in the folder. """
    if not os.path.exists(folder_path):
        print(f"Error: Input folder '{folder_path}' does not exist.")
        return

    # Filter for specific suffix
    target_suffix = "_temprature_1.0_nucleus_1.0_maxlen_300_intrasentence_predictions.json"
    json_files = [f for f in os.listdir(folder_path) if f.endswith(target_suffix)]

    if not json_files:
        print(f"No files ending with '{target_suffix}' found in the folder.")
        return
...
if __name__ == "__main__":
    # Configuration
    input_path = "toxicity_predictions"
