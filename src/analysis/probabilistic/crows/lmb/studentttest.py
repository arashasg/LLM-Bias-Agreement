...
def calculate_group_statistics(data, group_name):
    ...
    original_scores = [item["sentence_perplexity_score"] for item in data if item["group"] == group_name]
    replaced_scores = [item["replaced_sentence_perplexity_score"] for item in data if item["group"] == group_name]
...
def process_json_files(input_dir, output_dir):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith(".json"):
            ...
            # Detect unique groups in the data
            unique_groups = set(item["group"] for item in data)

            group_results = {}
            for group in unique_groups:
                stats_dict = calculate_group_statistics(data, group)
...
# Example usage
input_directory = "predictions/crowspairs"
output_directory = "output_updated/crows/lmb/per_model_scores"
